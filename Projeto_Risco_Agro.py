...
st.write("Versão 9.0 PRO: Régua Normativa de ICSD Integrada + Matriz de Política de Crédito")
...
# =========================================================================
# ABA 3: MOTOR DE CRÉDITO COM POLÍTICA NORMATIVA DE ICSD
# =========================================================================
with aba_motor:
    st.subheader("Configuração Financeira e Logística da Safra")
    
    # --- MODIFICAÇÃO SOLICITADA: TABELA DE REFERÊNCIA DA POLÍTICA DE ICSD ---
    with st.expander("📘 Visualizar Tabela Normativa de Referência ICSD (Mesa de Crédito)", expanded=False):
        df_politica_icsd = pd.DataFrame([
            {"Faixa de ICSD": "Abaixo de 1.00", "Classificação de Risco": "❌ CRÍTICO (Caixa Deficitário)", "Diretriz da Mesa": "Reprovação Automática da Proposta."},
            {"Faixa de ICSD": "1.00 a 1.19", "Classificação de Risco": "⚠️ ALERTA (Margem Estreita)", "Diretriz da Mesa": "Exigir Seguro Agrícola obrigatório + Aval de Terceiros."},
            {"Faixa de ICSD": "1.20 a 1.49", "Classificação de Risco": "✔️ SAUDÁVEL (Margem Confortável)", "Diretriz da Mesa": "Aprovação normal. Estruturação padrão de garantias."},
            {"Faixa de ICSD": "A partir de 1.50", "Classificação de Risco": "🌟 CONFORTÁVEL (Forte Resiliência)", "Diretriz da Mesa": "Aprovação prioritária. Elegível a bônus/redução de taxa."}
        ])
        st.table(df_politica_icsd)

    col1, col2 = st.columns(2)
    with col1:
        cultura = st.selectbox("Cultura Alvo do Financiamento", ["Soja", "Milho", "Trigo", "Triticale", "Sorgo"])
    ...
    if st.button("Executar Análise de Risco Completa", type="primary"):
        ...
        if status_esg != "REGULAR - Livre de Restrições":
            ...
            config_cultura = {
                ...
                "Triticale": {"produtividade": 45, "preco": 80, "inicio": "2025-05-01", "fim": "2025-06-30", "min_mm": 100},
            ...
            custo_producao_total = (area * prod_alvo * preco_saca_mercado * 0.60) + total_arrendamento
            ...
            # --- INTEGRAÇÃO DA RÉGUA DE ICSD NO RATING E VEREDITO DE POLÍTICA ---
            score_atual = resultado_serasa["score"] if resultado_serasa else 500
            
            # Classificação textual do ICSD com base na nossa régua
            if icsd < 1.00:
                laudo_icsd = "❌ CRÍTICO (Déficit)"
            elif icsd < 1.20:
                laudo_icsd = "⚠️ ALERTA (Apertado)"
            elif icsd < 1.50:
                laudo_icsd = "✔️ SAUDÁVEL (Adequado)"
            else:
                laudo_icsd = "🌟 CONFORTÁVEL (Forte)"
            
            taxa_base = 10.5
            if status_scr == "Restrição/Atraso > 60 dias" or total_divida_familiar > 500000 or score_atual < 400:
                spread_scr = 5.0; rating = "D - Alto Risco"; aprovado_scr = False
            elif status_scr == "Alta Alavancagem de Curto Prazo" or total_divida_familiar > 0 or score_atual < 600:
                spread_scr = 2.5; rating = "C - Risco Moderado"; aprovado_scr = True
            else:
                spread_scr = 0.0; rating = "A - Excelente"; aprovado_scr = True
                
            taxa_final = taxa_base + spread_scr - (1.5 if seguro_ativo else 0.0)
            
            # Reprova automaticamente se o ICSD for crítico (abaixo de 1.0), seguindo a regra
            credito_aprovado = (icsd >= 1.0) and aprovado_scr
            veredito_texto = "APROVADO" if credito_aprovado else "REPROVADO"

            salvar_analise_no_banco(
                nome, documento, localizacao, num_car, f"REGULAR (ICSD: {laudo_icsd})", 
            ...
            with res1:
                # Delta indica o laudo normativo da régua
            ...
            with res3:
                ...
                st.caption(f"Regime de Ocupação: {tipo_area.upper()}")
                
            st.write("---")
            if credito_aprovado: st.success(f"✔️ **CRÉDITO APROVADO.** Caixa enquadrado na régua normativa ({laudo_icsd}).")
            else: st.error(f"❌ **OPERAÇÃO REPROVADA PELA MESA.** Falha no enquadramento de margem ou restrição ativa.")
...
with aba_crm:
    ...
    if df_historico.empty:
        ...
        aprovados = len(df_historico[df_historico["veredito"] == "APROVADO"])
